...
void heapify_up(int min_heap[],int index)
{
    int parent_index=(index-1)/2;

    // If the current element is lesser than its parent, swap them
    if(index>0&&(min_heap[index]<min_heap[parent_index]))
    {
...
void insert(int val,int *size,int min_heap[])
{
...
int main()
{
    ...
    while (1)
    {
        ...
        cout<<"2.Get the minimum in the element"<<endl;
        ...
        if(choice==1)
        {
            int val;
            cin>>val;
            insert(val,&size,min_heap);
            cout<<"The element is inserted"<<endl;
        }
        else if(choice==2)
        {
            cout<<"the min is"<<(min_heap[0])<<endl;
        }
        else if(choice==3)
        {
            delete_the_element(min_heap,&size);
